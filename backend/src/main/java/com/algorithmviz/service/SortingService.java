...
public class SortingService {
    ...
    private List<SortStep> quickSort(List<Integer> input) {
        ...
        steps.add(buildSortStep(arr, allSorted, comparisons[0], swaps[0], accesses[0],
                List.of(), List.of(), null, 0, arr.length - 1, "✓ 快速排序完成！", 8));
        return steps;
    }
    ...
    private int partition(int[] arr, int low, int high,
        ...
        steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                ...
                "选择基准值 arr[" + high + "] = " + pivot + "，开始分区 [" + low + ", " + high + "]", 1));
            ...
            if (arr[j] <= pivot) {
                ...
                if (i != j) {
                    ...
                    steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                            ...
                            "交换 arr[" + i + "]↔arr[" + j + "]（现在为 " + arr[i] + ", " + arr[j] + "）", 5));
                }
            }
        }
        ...
        steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                ...
                "基准值归位：arr[" + pivotFinal + "] = " + arr[pivotFinal], 7));
        return pivotFinal;
    }
    ...
    private List<SortStep> mergeSort(List<Integer> input) {
        ...
        steps.add(buildSortStep(arr, allSorted, cmp[0], swp[0], acc[0],
                List.of(), List.of(), null, 0, arr.length - 1, "✓ 归并排序完成！", 7));
        return steps;
    }
    ...
    private void mergeSortHelper(int[] arr, int left, int right,
        ...
        steps.add(buildSortStep(arr, List.of(), cmp[0], swp[0], acc[0],
                ...
                "分割 [" + left + ".." + right + "] → [" + left + ".." + mid + "] 和 [" + (mid + 1) + ".." + right + "]", 1));
    ...
    private void merge(int[] arr, int left, int mid, int right,
        ...
        steps.add(SortStep.builder()
                ...
                .codeLine(2).comparisons(cmp[0]).swaps(swp[0]).accesses(acc[0]).build());
        ...
        while (i < leftArr.length && j < rightArr.length) {
            ...
            steps.add(SortStep.builder()
                    ...
                    .codeLine(4).comparisons(cmp[0]).swaps(swp[0]).accesses(acc[0]).build());
            k++;
        }
        while (i < leftArr.length) {
            ...
            steps.add(SortStep.builder()
                    ...
                    .codeLine(5).comparisons(cmp[0]).swaps(swp[0]).accesses(acc[0]).build());
            k++;
        }
        while (j < rightArr.length) {
            ...
            steps.add(SortStep.builder()
                    ...
                    .codeLine(6).comparisons(cmp[0]).swaps(swp[0]).accesses(acc[0]).build());
            k++;
        }
    }
        ...
        for (int i = 0; i < arr.length - 1; i++) {
            ...
            for (int j = 0; j < arr.length - 1 - i; j++) {
                ...
                steps.add(buildSortStep(arr, sorted, cmp, swp, acc,
                        ...
                        "比较 arr[" + j + "]=" + arr[j] + " 与 arr[" + (j + 1) + "]=" + arr[j + 1], 2));
                if (arr[j] > arr[j + 1]) {
                    ...
                    steps.add(buildSortStep(arr, sorted, cmp, swp, acc,
                            ...
                            "交换 arr[" + j + "]↔arr[" + (j + 1) + "]（现在为 " + arr[j] + ", " + arr[j + 1] + "）", 3));
                }
            }
            sorted.add(arr.length - 1 - i);
            steps.add(buildSortStep(arr, sorted, cmp, swp, acc,
                    List.of(), List.of(), null, 0, arr.length - 1 - i,
                    "第 " + (arr.length - 1 - i) + " 轮结束，元素 " + arr[arr.length - 1 - i] + " 已归位", 4));
            if (!swapped) break;
        }

        List<Integer> allSorted = IntStream.range(0, arr.length).boxed().collect(Collectors.toList());
        steps.add(buildSortStep(arr, allSorted, cmp, swp, acc,
                List.of(), List.of(), null, 0, arr.length - 1, "✓ 冒泡排序完成！", 5));
        return steps;
    }
    ...
    private List<SortStep> heapSort(List<Integer> input) {
        ...
        for (int i = n - 1; i > 0; i--) {
            ...
            steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                    ...
                    "将堆顶 " + arr[i] + " 移至末尾，堆大小减为 " + i, 5));
            heapify(arr, i, 0, sorted, cmp, swp, acc, steps);
        }
        sorted.add(0);
        List<Integer> allSorted = IntStream.range(0, arr.length).boxed().collect(Collectors.toList());
        steps.add(buildSortStep(arr, allSorted, cmp[0], swp[0], acc[0],
                List.of(), List.of(), null, 0, arr.length - 1, "✓ 堆排序完成！", 7));
        return steps;
    }
    ...
    private void heapify(int[] arr, int n, int i,
        ...
        if (l < n) {
            ...
            steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                    ...
                    "比较左子节点 arr[" + l + "]=" + arr[l] + " 与父节点 arr[" + largest + "]=" + arr[largest], 2));
        ...
        if (r < n) {
            ...
            steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                    ...
                    "比较右子节点 arr[" + r + "]=" + arr[r] + " 与当前最大 arr[" + largest + "]=" + arr[largest], 3));
        ...
        if (largest != i) {
            ...
            steps.add(buildSortStep(arr, sorted, cmp[0], swp[0], acc[0],
                    ...
                    "交换 arr[" + i + "]↔arr[" + largest + "]（" + arr[i] + " ↔ " + arr[largest] + "）", 4));
    ...
    private List<SortStep> insertionSort(List<Integer> input) {
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            steps.add(buildSortStep(arr, sorted, cmp, swp, acc,
                    ...
                    "将 key=" + key + " 插入到位置 " + (j + 1), 4));
        }

        List<Integer> allSorted = IntStream.range(0, arr.length).boxed().collect(Collectors.toList());
        steps.add(buildSortStep(arr, allSorted, cmp, swp, acc,
                List.of(), List.of(), null, 0, arr.length - 1, "✓ 插入排序完成！", 5));
        return steps;
    }
    ...
    private SortStep buildSortStep(int[] arr, List<Integer> sorted,
                                   ...
                                   String description, int codeLine) {
